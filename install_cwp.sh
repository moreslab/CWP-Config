...
wget https://raw.githubusercontent.com/moreslab/CWP-Config/main/configure_centos.sh -O "$CWD/configure_centos.sh" && bash "$CWD/configure_centos.sh"
...
find /usr/local/php/ -name "php.ini" | xargs sed -i 's/^date.timezone.*/date.timezone = "Europe\/Istanbul"/g'
...
ntpdate 0.pool.ntp.org 1.pool.ntp.org 2.pool.ntp.org 3.pool.ntp.org 0.tr.pool.ntp.org
if [ -f /usr/share/zoneinfo/Europe/Istanbul ]; then
        echo "Seteando timezone a Europe/Istanbul..."
